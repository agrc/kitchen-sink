...
exports.UgrcLogo = exports.Header = void 0;
...
var Header = function (_a) {
    var children = _a.children, links = _a.links, props = __rest(_a, ["children", "links"]);
    var state = (0, react_stately_1.useOverlayTriggerState)(props);
    var _b = (0, react_aria_1.useOverlayTrigger)({ type: 'dialog' }, state), triggerProps = _b.triggerProps, overlayProps = _b.overlayProps;
    var currentUser = props.currentUser, logout = props.logout;
    return (<header className="w-full">
      <div className="pointer-events-none absolute z-50 flex w-full items-center justify-center overflow-hidden bg-transparent">
        <div className="pointer-events-none translate-y-[-150%] rounded-b border border-t-0 bg-zinc-200 leading-6 transition-all duration-500 ease-in-out focus-within:translate-y-0 focus-within:border-black/30 focus-within:shadow-lg dark:bg-zinc-700">
          <a href="#main-content" className="relative mx-6 my-2 flex cursor-pointer items-center justify-center rounded-full border-2 border-primary-800 bg-primary-800 px-8 py-2 text-white">
      ...
      <div className="flex items-center border-b border-b-primary-900/30 bg-white py-2 pr-3 dark:border-black/30 dark:bg-zinc-700/20">
        <div className="flex h-12 grow space-x-4 divide-x divide-primary-950 pl-3 dark:divide-zinc-600 sm:space-x-6 md:h-16">
          <div className="flex items-center rounded-md transition-shadow ease-in-out focus-within:ring-2 focus-within:ring-primary-900 focus-within:ring-offset-2 dark:ring-offset-transparent dark:focus-within:ring-secondary-600 md:px-3">
            <react_aria_components_1.Button {...triggerProps} onKeyDown={function (event) { return dismiss(event, state.close); }} className="cursor-default focus:outline-none">
        ...
        <div className="flex shrink items-center justify-end gap-6 lg:mr-10">
          {links && (<react_aria_components_1.MenuTrigger>
              <div className="flex items-center justify-self-end">
                <react_aria_components_1.Button aria-label="user menu" className="cursor-default rounded-md outline-none transition-shadow ease-in-out focus:ring-2 focus:ring-primary-900 focus:ring-offset-2 dark:ring-offset-transparent dark:focus:ring-secondary-600">
                  <lucide_react_1.GripIcon className="h-full w-8 text-primary-900 dark:text-zinc-300"/>
                </react_aria_components_1.Button>
                <Menu_1.Menu items={links}>
                  {function (link) { return (<Menu_1.MenuItem href={link.action.url} target={link.action.url.toLowerCase().startsWith('https')
                    ? '_blank'
                    : undefined}>
                      <span className="flex items-center justify-between rounded-md p-1 text-sm">
                        {link.key}
                        {link.action.url.toLowerCase().startsWith('https') ? (<>
                            <lucide_react_1.SquareArrowOutUpRightIcon className="ml-1 h-full w-4"/>
                          </>) : null}
                      </span>
                    </Menu_1.MenuItem>); }}
                </Menu_1.Menu>
          ...
          {currentUser && (<react_aria_components_1.MenuTrigger>
              <react_aria_components_1.Button aria-label="secondary menu" className="cursor-default rounded-md outline-none transition-shadow ease-in-out focus:ring-2 focus:ring-primary-900 focus:ring-offset-2 dark:ring-offset-transparent dark:focus:ring-secondary-600">
                <Avatar_1.Avatar anonymous={!(currentUser !== null && currentUser !== void 0 ? currentUser : false)} user={currentUser}/>
              </react_aria_components_1.Button>
              <Menu_1.Menu>
                <Menu_1.MenuItem href="https://id.utah.gov">
                  <span className="flex items-center justify-between">
                    Utahid account
                    <lucide_react_1.SquareArrowOutUpRightIcon className="ml-1 h-full w-4"/>
                  </span>
                </Menu_1.MenuItem>
                <Menu_1.MenuItem href="https://gravatar.com">
                  <span className="flex items-center justify-between">
                    Gravatar account
                    <lucide_react_1.SquareArrowOutUpRightIcon className="ml-1 h-full w-4"/>
                  </span>
                ...
                <Menu_1.MenuItem onAction={logout}>Sign out</Menu_1.MenuItem>
              </Menu_1.Menu>
            </react_aria_components_1.MenuTrigger>)}
        </div>
      </div>
...
var Flyout = function (_a) {
    var state = _a.state, props = __rest(_a, ["state"]);
    var ref = (0, react_1.useRef)(null);
    var modalProps = (0, react_aria_1.useModalOverlay)(__assign(__assign({}, props), { isKeyboardDismissDisabled: false, isDismissable: true }), state, ref).modalProps;
    return (<>
      <div {...modalProps} ref={ref} className="relative z-10 bg-zinc-700 px-6 pb-6 pt-2 shadow-lg">
        ...
        <div className="mx-auto flex max-w-screen-lg flex-col justify-around gap-10 lg:flex-row">
          <div className="flex items-center gap-2 lg:w-96 lg:items-start">
            <lucide_react_1.ShieldCheckIcon className="w-16 text-zinc-50"/>
          ...
          <div className="flex items-center gap-2 lg:w-96 lg:items-start">
            <lucide_react_1.LockIcon className="w-16 text-zinc-50"/>
        ...
        <react_aria_components_1.Button onPress={state.close} className="absolute right-2 top-2 cursor-default rounded-full p-1 text-zinc-50 outline-none ring-offset-transparent transition-shadow ease-in-out hover:bg-black/10 focus:ring-2 focus:ring-secondary-600 focus:ring-offset-2">
          <lucide_react_1.XIcon className="w-7"/>
          <span className="sr-only">Close official website popup</span>
        </react_aria_components_1.Button>
      </div>
    </>);
};
var UgrcLogo = function () { return (<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="hidden h-full max-w-52 text-zinc-600 sm:block sm:max-w-none" viewBox="0 0 64 64">
    <defs>
      <clipPath id="a">
    ...
    <path fill="gray" stroke="gray" strokeMiterlimit={10} d="M32.65 63.46a31.29 31.29 0 0 1-22.25-9.21A31.23 31.23 0 0 1 1.19 32 31.29 31.29 0 0 1 10.4 9.75 31.23 31.23 0 0 1 32.65.54 31.29 31.29 0 0 1 54.9 9.75 31.23 31.23 0 0 1 64.11 32a31.29 31.29 0 0 1-9.21 22.25 31.23 31.23 0 0 1-22.25 9.21Zm0-62.87a31.22 31.22 0 0 0-22.21 9.2A31.28 31.28 0 0 0 1.24 32a31.22 31.22 0 0 0 9.2 22.21 31.28 31.28 0 0 0 22.21 9.2 31.22 31.22 0 0 0 22.21-9.2A31.28 31.28 0 0 0 64.06 32a31.22 31.22 0 0 0-9.2-22.21A31.28 31.28 0 0 0 32.65.59Z"/>
  </svg>); };
exports.UgrcLogo = UgrcLogo;
var UtahLogo = function (_a) {
    var className = _a.className;
    return (<svg viewBox="0 0 105.9496 47.6226" role="img" className={className}>
    <g>
      <g>
