...
export const Footer = ({
  ...
  columnOne = ugrcStandardLinks.columnOne,
  columnTwo = ugrcStandardLinks.columnTwo,
  columnThree = ugrcStandardLinks.columnThree,
}: FooterProps) => (
  <>
    <section>
        ...
        <div>
          ...
          <ul className="grid gap-2 pl-1 text-sm leading-loose md:gap-0">
            {columnOne.links.map((link, index) => (
              <li
                className={index === 0 ? 'mt-4 block' : undefined}
                key={link.title}
              >
                <Link href={link.url} variant="secondary" quiet={true}>
                  {link.title}
                </Link>
              </li>
            ))}
    ...
    <div>
      <div className="max-w-xs">
        <img
          ...
          width="100"
          height="115"
          alt="Utah Department of Natural Resources logo"
          loading="lazy"
        />
      </div>
      <div className="mt-4 text-lg">
