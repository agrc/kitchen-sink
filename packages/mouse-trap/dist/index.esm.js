...
const projectPoint = (viewPoint, options = {
  projectToWkid: 4326,
  decimals: 3
}) => {
  const projected = project(viewPoint, {
    wkid: options.projectToWkid
  });
  if (!projected) {
...
const MouseTrap = ({
  ...
  useEffect(() => {
    ...
    let handle = null;
    load().then(() => {
      handle = mapView.on("pointer-move", throttle((evt) => setLocation(projectPoint(mapView.toMap(evt), options)), options.wait));
    });
    return () => {
      if (handle) {
        handle.remove();
